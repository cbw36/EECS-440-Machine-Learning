...
def cv_logreg(exset):
    ...
    for i in range(folds):
        ...
        # TODO: Update
        conditional_probabilities = build_logreg(train_exset)

        accuracy, precision, recall, conf_and_label = run_logreg(test_exset)
...
def full_logreg(exset_ar, num_bins, m_estimate, attributes):
...
# TODO: Populate
def build_logreg(exset):
    return 0

def run_logreg(exset):
    fp = 0
    tp = 0
    fn = 0
    tn = 0
    ...
    for ex in exset:
        true_label = ex[-1]
        predicted_label, predicted_confidence = iterate_example(ex)
